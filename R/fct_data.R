...
get_pinned_data <- function(
  ...
  if (!pin_exists) {
    stop(glue::glue("The pin {pin_name} could not be found"))
  }
...
wrangle_studies <- function(pinned_data) {
  ...
  outcomes_reported_i <- which(
    colnames(studies_selected) == "Outcomes reported"
  )
...
wrangle_reviews <- function(pinned_data) {
  ...
  outcomes_reported_i <- which(
    colnames(reviews_selected) == "Outcomes reported"
  )
